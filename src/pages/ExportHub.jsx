...
export default function ExportHub() {
  const { data, columns, actions, fileName, planLimits, userTier } = useData();
        ...
        actions.forEach((action, idx) => {
            ...
            switch (action.type) {
                ...
                case 'AFFIX':
                    ...
                    else steps += `            if '${action.col}' in df.columns: df['${action.col}'] = df['${action.col}'] + '${action.text}'\n`;
                    break;
                case 'SUBSTR':
        ...
        <div className="p-6 border-b border-gray-200 dark:border-wolf/20 bg-white dark:bg-carbon-light">
           ...
           <div className="flex gap-8 mt-8 border-b border-gray-200 dark:border-wolf/10 text-sm">
              ...
              <button onClick={() => setActiveTab('sql')} className={`pb-3 px-1 font-bold transition-all border-b-2 flex items-center gap-2 ${activeTab === 'sql' ? 'text-persian border-persian' : 'text-gray-400 border-transparent hover:text-gray-600 dark:text-wolf dark:hover:text-white'}`}>
                <Database size={16}/> 2. SQL Generator {userTier === 'free' && <Lock size={12} className="text-gray-400"/>}
              </button>
              <button onClick={() => setActiveTab('code')} className={`pb-3 px-1 font-bold transition-all border-b-2 flex items-center gap-2 ${activeTab === 'code' ? 'text-persian border-persian' : 'text-gray-400 border-transparent hover:text-gray-600 dark:text-wolf dark:hover:text-white'}`}>
                <Terminal size={16}/> 3. Python App {userTier === 'free' && <Lock size={12} className="text-gray-400"/>}
              </button>
           </div>
        </div>
            ...
            <>
              ...
              {activeTab === 'sql' && (
                <div className="h-full flex flex-col gap-4 relative animate-in fade-in">
                  {!planLimits.exportCode && <ProLockOverlay />}
                  <div className="flex justify-between items-center mb-2 px-1">
                      <div className="text-xs font-mono text-gray-500 dark:text-wolf flex items-center gap-2"><Database size={12}/> script.sql</div>
                      <button onClick={() => copyToClipboard(generateSQL())} className="flex items-center gap-2 text-xs font-bold bg-persian hover:bg-sea text-white px-4 py-2 rounded-lg transition-all shadow-lg shadow-persian/20 active:scale-95">{copied ? <Check size={14} /> : <Copy size={14} />} {copied ? 'Copiado' : 'Copiar SQL'}</button>
                  </div>
                  <div className="flex-1 bg-[#1e1e1e] rounded-xl p-6 overflow-auto border border-gray-700 custom-scrollbar shadow-inner text-left relative group">
                    <pre className="text-sm font-mono text-zinc whitespace-pre-wrap leading-relaxed"><code className="language-sql">{generateSQL()}</code></pre>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'code' && (
                <div className="h-full flex flex-col gap-4 relative animate-in fade-in">
                  {!planLimits.exportCode && <ProLockOverlay />}
                  <div className="flex justify-between items-center mb-2 px-1">
                      <div className="text-xs font-mono text-gray-500 dark:text-wolf flex items-center gap-2"><Terminal size={12}/> app.py (Tkinter + Pandas)</div>
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => copyToClipboard(generatePythonGUI())} className="flex items-center gap-2 text-xs font-bold bg-persian hover:bg-sea text-white px-4 py-2 rounded-lg transition-all shadow-lg shadow-persian/20 active:scale-95">{copied ? <Check size={14} /> : <Copy size={14} />} {copied ? 'Copiado' : 'Copiar Código'}</button>
                      </div>
                  </div>
                  <div className="flex-1 bg-[#1e1e1e] rounded-xl p-6 overflow-auto border border-gray-700 custom-scrollbar shadow-inner text-left relative">
                    <pre className="text-sm font-mono whitespace-pre-wrap leading-relaxed text-zinc">{generatePythonGUI()}</pre>
                  </div>
                </div>
              )}
            </>
