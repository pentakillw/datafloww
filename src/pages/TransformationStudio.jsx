...
export default function TransformationStudio() {
  ...
  const [scanScope, setScanScope] = useState('all'); // 'all' | 'selection'

  useEffect(() => {
    if (columns.length > 0) {
      setMathCol2(prev => prev || columns[0]);
      setMergeCol2(prev => prev || columns[0]);
      setCompCol2(prev => prev || columns[0]);
      setGroupAggCol(prev => prev || columns[0]);
    }
  }, [columns]);

  // Manejador de clicks fuera
  useEffect(() => {
    function handleClickOutside(event) {
      if (menuRef.current && !menuRef.current.contains(event.target)) {
        setOpenMenu(null);
      }
      if (headerMenuOpen && !event.target.closest('.header-menu-container')) {
  ...
  // --- LOGICA MODAL CONDICIONAL ---
  const addCondRule = () => {
      setCondRules([...condRules, { col: columns[0], op: 'equals', val: '', output: '' }]);
  };
  const updateCondRule = (idx, field, val) => {
      const newRules = [...condRules];
      newRules[idx][field] = val;
      setCondRules(newRules);
  };
  const removeCondRule = (idx) => {
      setCondRules(condRules.filter((_, i) => i !== idx));
  };

  // --- LOGICA MODAL "A PARTIR DE EJEMPLOS" ---
  const runExampleInference = (currentMap, scope) => {
      // Determinar columnas a escanear
      const colsToScan = (scope === 'selection' && activeCol) ? [activeCol] : columns;
      
      // Trigger Inferencia en el Hook
      const rule = transform.inferTransformation(data, currentMap, colsToScan);
      setInferredRule(rule);

      if (rule) {
          // Predecir el resto de filas usando la regla inferida
          const preds = {};
          data.forEach((row, idx) => {
              if (currentMap[idx] !== undefined) return; // Ya tiene valor manual
              const val = transform.applyRuleToRow(row, rule);
              preds[idx] = val;
          });
  ...
  return (
    <div className="h-full flex items-start p-3 bg-gray-50/50 dark:bg-black/20 animate-in fade-in zoom-in-95 duration-300 relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-carbon relative z-10 rounded-xl border border-gray-200 dark:border-wolf/10 shadow-sm h-full"> 
        ...
        {/* NAVBAR PRINCIPAL */}
        <div className="bg-white dark:bg-carbon-light border-b border-gray-200 dark:border-wolf/20 p-2 flex gap-2 items-center relative z-50 flex-wrap shadow-sm rounded-t-xl select-none" ref={menuRef}>
          ...
          <DropdownMenu label="Agregar Columna" icon={<PlusSquare size={16} className="text-persian" />} isOpen={openMenu === 'add_col'} onClick={() => toggleMenu('add_col')}>
            ...
            <DropdownItem icon={<PlayCircle size={14} />} label="Invocar función personalizada" onClick={() => { showToast('Usa "Transformar" para aplicar funciones predefinidas.', 'info'); setOpenMenu(null); }} />
        ...
        {/* --- MODALES --- */}
        {/* MODAL: COLUMNA POR EJEMPLOS (MEJORADO) */}
                ...
                <div className="bg-white dark:bg-[#1a1a1a] rounded-xl shadow-2xl w-full max-w-5xl h-[80vh] border border-gray-200 dark:border-wolf/20 flex flex-col">
                    <div className="p-4 border-b border-gray-200 dark:border-wolf/10 flex justify-between items-center bg-gray-50 dark:bg-white/5">
                        <div className="flex items-center gap-4">
                            ...
                            {/* SELECTOR DE SCOPE (ALL vs SELECTION) */}
                            <div className="flex items-center bg-white dark:bg-carbon border border-gray-200 dark:border-wolf/20 rounded-lg p-1 ml-4">
                                <button 
                                    ...
                                    className={`px-3 py-1 text-xs font-bold rounded-md transition-colors flex items-center gap-2 ${scanScope === 'all' ? 'bg-persian text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5'}`}
                                >
                                    <ScanSearch size={14} /> Todas las columnas
                                </button>
                                <button 
                                    onClick={() => handleScopeChange('selection')}
                                    disabled={!activeCol}
                                    title={!activeCol ? "Selecciona una columna primero" : ""}
                                    className={`px-3 py-1 text-xs font-bold rounded-md transition-colors flex items-center gap-2 ${scanScope === 'selection' ? 'bg-persian text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 disabled:opacity-50'}`}
                                >
                                    <MousePointerClick size={14} /> Solo {activeCol || 'selección'}
                                </button>
                            </div>
                        </div>
                        ...
                        <table className="w-full text-left border-collapse border border-gray-200 dark:border-wolf/10">
                            <thead className="bg-gray-100 dark:bg-black/20 sticky top-0 z-10">
                                <tr>
                                    ...
                                    {/* Mostrar solo la columna activa si el scope es 'selection', o todas si es 'all' */}
                                    {columns.map(c => (
                                        <th key={c} className={`p-2 text-xs font-bold border border-gray-200 dark:border-wolf/10 text-gray-500 ${scanScope === 'selection' && activeCol === c ? 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-600' : ''} ${scanScope === 'selection' && activeCol !== c ? 'opacity-30' : ''}`}>
                                            {c}
                                        </th>
                                    ))}
                                    <th className="p-2 text-xs font-bold border border-gray-200 dark:border-wolf/10 bg-persian/10 text-persian w-48 border-l-2 border-l-persian">
                                        Columna 1 (Ejemplos)
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {data.slice(0, 20).map((row, i) => (
                                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-white/5">
                                        ...
                                        {columns.map(c => (
                                            <td key={c} className={`p-2 text-xs border border-gray-200 dark:border-wolf/10 text-gray-600 dark:text-zinc ${scanScope === 'selection' && activeCol !== c ? 'opacity-30' : 'opacity-70'}`}>
                                                {String(row[c] || '')}
                                            </td>
                                        ))}
                                        <td className="p-0 border border-gray-200 dark:border-wolf/10 border-l-2 border-l-persian relative">
                                            <input 
                                                type="text" 
                                                className="w-full h-full p-2 text-sm bg-transparent outline-none focus:bg-persian/5 text-gray-900 dark:text-white placeholder:text-gray-300 dark:placeholder:text-gray-600"
                                                placeholder={previewExampleCol[i]}
                                                value={exampleMap[i] || ''}
                                                onChange={(e) => handleExampleChange(i, e.target.value)}
                                            />
                                            {/* Indicador visual de predicción */}
                                            {!exampleMap[i] && previewExampleCol[i] && (
                                                <span className="absolute right-2 top-2 text-[10px] text-gray-300 italic pointer-events-none">auto</span>
                                            )}
                                        </td>
                                    </tr>
                                ))}
                    ...
                    <div className="p-4 border-t border-gray-200 dark:border-wolf/10 flex justify-between items-center bg-gray-50 dark:bg-white/5 rounded-b-xl">
                        <div className="text-xs">
                            {inferredRule ? (
                                <span className="text-green-600 font-bold flex items-center gap-2">
                                    ...
                                    Patrón: {inferredRule.type === 'template' ? `Combinación` : inferredRule.type.toUpperCase()}
                                </span>
                            ) : (
                                <span className="text-gray-400">Escribe ejemplos para detectar el patrón...</span>
                            )}
                        </div>
                        <div className="flex gap-2">
                            ...
                            <button onClick={() => { transform.generateColumnFromExamples(customColName, exampleMap); closeModal(); }} disabled={!inferredRule} className="btn-primary-sm">Aceptar</button>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div className="flex gap-4 h-64">
                                <div className="w-1/3 border border-gray-200 dark:border-wolf/10 rounded-lg overflow-hidden flex flex-col">
                                    ...
                                    <div className="flex-1 overflow-y-auto p-1 bg-white dark:bg-black/20">
                                        {columns.map(c => (
                                            <button key={c} onClick={() => setCustomFormula(prev => prev + `row['${c}']`)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-persian/10 text-gray-700 dark:text-zinc rounded truncate" title="Clic para insertar">
                                                {c}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="flex-1 flex flex-col">
                                    <textarea 
                                        className="flex-1 w-full bg-[#1e1e1e] text-green-400 font-mono text-sm p-3 rounded-lg border border-gray-700 focus:border-persian focus:ring-1 focus:ring-persian resize-none leading-relaxed"
                                        value={customFormula}
                                        onChange={e => setCustomFormula(e.target.value)}
                                        placeholder="// Escribe tu fórmula JS aquí...&#10;return row['Precio'] * 1.16;"
                                    ></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            {condRules.map((rule, idx) => (
                                <div key={idx} className="flex flex-wrap items-center gap-2 bg-gray-50 dark:bg-white/5 p-3 rounded-lg border border-gray-200 dark:border-wolf/10">
                                    ...
                                    <select className="input-dark !w-32" value={rule.col} onChange={e => updateCondRule(idx, 'col', e.target.value)}>
                                        {columns.map(c => <option key={c} value={c}>{c}</option>)}
                                    </select>
                                    <select className="input-dark !w-32" value={rule.op} onChange={e => updateCondRule(idx, 'op', e.target.value)}>
                                    ...
                                    {!['is_null', 'not_null'].includes(rule.op) && (
                                        <input type="text" className="input-dark !w-32" placeholder="Valor" value={rule.val} onChange={e => updateCondRule(idx, 'val', e.target.value)} />
                                    )}
        ...
        {activeModal === 'conditional' && <ModalBar title="Condicional" onClose={closeModal}><select className="input-dark w-12 text-center" value={condOp} onChange={e => setCondOp(e.target.value)}><option value=">">{'>'}</option><option value="<">{'<'}</option><option value="=">=</option><option value="!=">!=</option></select><input type="text" className="input-dark w-16" value={condVal} onChange={e => setCondVal(e.target.value)} placeholder="Valor" /><span className="text-xs">?</span><input type="text" className="input-dark w-16" value={condTrue} onChange={e => setCondTrue(e.target.value)} placeholder="True" /><span className="text-xs">:</span><input type="text" className="input-dark w-16" value={condFalse} onChange={e => setCondFalse(e.target.value)} placeholder="False" /><input type="text" className="input-dark w-20 font-bold text-persian" value={condTarget} onChange={e => setCondTarget(e.target.value)} placeholder="Nombre" /><button onClick={() => { transform.applyConditional(activeCol, condOp, condVal, condTrue, condFalse, condTarget); closeModal(); }} className="btn-primary-sm">Ok</button></ModalBar>}
        {activeModal === 'compare' && <ModalBar title="Comparar" onClose={closeModal}><span className="text-xs">Con:</span><select className="input-dark w-24" value={compCol2} onChange={e=>setCompCol2(e.target.value)}>{columns.filter(c=>c!==activeCol).map(c=><option key={c} value={c}>{c}</option>)}</select><button onClick={() => { transform.compareColumns(activeCol, compCol2); closeModal(); }} className="btn-primary-sm">Comparar</button></ModalBar>}
        {activeModal === 'binning' && <ModalBar title="Binning (Rangos)" onClose={closeModal}><span className="text-xs">Tamaño:</span><input type="number" className="input-dark w-16" value={binSize} onChange={e=>setBinSize(e.target.value)} /><button onClick={() => { transform.createBinning(activeCol, binSize); closeModal(); }} className="btn-primary-sm">Crear</button></ModalBar>}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider justify-between">
                          ...
                          {/* BOTÓN MENÚ CONTEXTUAL DE COLUMNA */}
                          <button 
                            onClick={(e) => toggleHeaderMenu(col, e)} 
                            className={`p-1 rounded hover:bg-gray-300 dark:hover:bg-wolf/20 transition-colors opacity-50 group-hover:opacity-100 ${headerMenuOpen === col ? 'opacity-100 bg-gray-200 dark:bg-wolf/20 text-persian' : ''}`}
                            title="Opciones de columna"
                          >
                             <MoreVertical size={14} />
                          </button>

                          {/* DROPDOWN DE COLUMNA (MENU CONTEXTUAL) */}
                          {headerMenuOpen === col && (
                            <div className="header-menu-container absolute top-full right-0 mt-1 w-48 bg-white dark:bg-[#1a1a1a] rounded-lg shadow-xl border border-gray-200 dark:border-wolf/20 z-50 flex flex-col py-1 text-gray-700 dark:text-zinc text-xs font-normal normal-case tracking-normal animate-in fade-in zoom-in-95 duration-200 cursor-default" onClick={(e) => e.stopPropagation()}>
                                
                                <button onClick={() => { transform.sortData(col, 'asc'); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                  <ArrowDownAZ size={14} className="text-gray-400" /> Ordenar A-Z
                                </button>
                                <button onClick={() => { transform.sortData(col, 'desc'); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                  <ArrowUpAZ size={14} className="text-gray-400" /> Ordenar Z-A
                                </button>
                                
                                <div className="border-t border-gray-100 dark:border-wolf/10 my-1"></div>
                                
                                <button onClick={() => { transform.moveColumn(col, 'left'); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                  <ArrowLeft size={14} className="text-gray-400" /> Mover Izquierda
                                </button>
                                <button onClick={() => { transform.moveColumn(col, 'right'); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                  <ArrowRight size={14} className="text-gray-400" /> Mover Derecha
                                </button>
                                
                                <button onClick={() => { transform.duplicateColumn(col); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                ...
                                <button onClick={() => { setNewName(col); setActiveModal('rename'); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                ...
                                <button onClick={() => { transform.fillDown(col); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                  <ArrowDownToLine size={14} className="text-gray-400" /> Rellenar Abajo
                                </button>
                                <button onClick={() => { transform.trimText(col); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-gray-100 dark:hover:bg-wolf/10 flex items-center gap-2 text-left">
                                  <Scissors size={14} className="text-gray-400" /> Trim Espacios
                                </button>

                                <div className="border-t border-gray-100 dark:border-wolf/10 my-1"></div>

                                <button onClick={() => { transform.dropColumn(col); setHeaderMenuOpen(null); }} className="px-3 py-2 hover:bg-red-50 dark:hover:bg-red-900/10 flex items-center gap-2 text-left text-red-500">
...
const DropdownMenu = ({ label, icon, children, isOpen, onClick }) => {
  ...
  const items = React.Children.toArray(children).filter(child => {
    ...
    if (child.props.title) return true; // Always show section titles
